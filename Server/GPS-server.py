...
size_of_track_w = 485#530 #cm
size_of_track_h = 400 #cm
...
frame_init = cv2.imread("Track/Merged_frame_Aruco3.jpg")
...
interval_x = 10*(bottom_right_track[0] - top_left_track[0])/size_of_track_w
interval_y = 10*(bottom_right_track[1] - top_left_track[1])/size_of_track_h

for k in range (int(top_left_track[0]), int(bottom_right_track[0]), int(interval_x)):
            cv2.line(frame_init, (k, int(top_left_track[1])), (k, int(bottom_right_track[1])), (255, 0, 0), 1, 1)

for p in range (int(top_left_track[1]), int(bottom_right_track[1]), int(interval_y)):
    cv2.line(frame_init, (int(top_left_track[0]), p), (int(bottom_right_track[0]), p), (255, 0, 0), 1, 1)

cv2.imwrite("Merged_frame_points.jpg", frame_init)
...
frame1 = cv2.imread("Track/Merged_frame_Aruco_car"+str(1)+".jpg")
...
for j in range (1, 6):
    ...
    time.sleep(0.5)
    frame = cv2.imread("Track/Merged_frame_Aruco_car"+str(j)+".jpg")
    #success, frame = cap.read()
    ...
    if point_car[0] > 0 and point_car[1] > 0:
        ...
        actual_gps_x = x_offset + gps_x * size_of_track_w / ((bottom_right_track[0]) - (top_left_track[0]))
        ...
        for p in range (int(top_left_track[1]), int(bottom_right_track[1]), int(interval_y)):
            cv2.line(frame, (int(top_left_track[0]), p), (int(bottom_right_track[0]), p), (255, 0, 0), 1, 1)

        
        cv2.imshow("Frames", frame)
        count_frames += 1
        cv2.imwrite("Track/test_track_"+str(count_frames)+".jpg", frame)
