...
class GPS:
    def __init__(self):
        ### Track Params ###
        self.size_of_track_w = 160 #cm
        self.size_of_track_h = 180 #cm
        self.init_x = 0 #cm
        self.init_y = 0 #cm
    ...
    def tracking_procedure(self, img, countFrames):
        ...
        if countFrames == 1:
            ...
            interval_x = 10*(top_right_track[0] - top_left_track[0])/self.size_of_track_w
            interval_y = 10*(bottom_right_track[1] - top_left_track[1])/self.size_of_track_h
        ...
        if countFrames >= 1:
            ...
            if point_car[0] > 0 and point_car[1] > 0:
                ...
                actual_gps_x = self.init_x + (gps_x * self.size_of_track_w / (self.top_right_track[0] - self.top_left_track[0]))
                actual_gps_y = self.init_y + (gps_y * self.size_of_track_h / (self.bottom_right_track[1] - self.top_left_track[1]))
                cv2.putText(img, "X: "+str(int(actual_gps_x))+" & Y: "+str(int(actual_gps_y)), (int(gps_x), int(gps_y)), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (0, 0, 255), 3)

            
            for k in range (int(self.top_left_track[0]), int(self.top_right_track[0]), int(self.interval_x)):
            ...
            for p in range (int(self.top_left_track[1]), int(self.bottom_right_track[1]), int(self.interval_y)):
                cv2.line(img, (int(self.top_left_track[0]), p), (int(self.top_right_track[0]), p), (255, 0, 0), 1, 1)
